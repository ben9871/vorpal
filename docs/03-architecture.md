...
```
audiobooker/                      # package (name provisional)
...
- Human edits happen **only** in `chapters` (titles, `include`, boundaries,
  `spoken_intro`). `audiobook review` presents the table, the user edits, downstream
...
`audiobook review book.pdf` prints the chapter table (id, kind, title, pages, words,
source, confidence, include) plus any flagged pages, and opens `book.json` for editing.
`audiobook build` proceeds automatically when overall confidence is high (outline/TOC
...
- **Chunking:** sentences packed to ~400 chars, never splitting a sentence across chunks;
  paragraph boundaries recorded so synthesis can insert a longer pause.
...
Artifact: `chunks/{chapter}.jsonl` — `{ idx, text, pause_after_ms, text_hash }`.

## Stage 6 — `synth`: never silently drop a sentence

- `TTSEngine` interface: `synthesize(text) -> (samples, sample_rate)`; `kokoro_engine.py`
  is the default (CPU/GPU auto). Engines declare max chunk length; the normalizer respects
  it. *(No voice cloning — out of scope per the vision doc.)*
...
- **Cache:** chunk WAVs keyed by `(text_hash, engine, voice, speed)` — edits invalidate
  precisely the chunks whose text changed.
