...
> **Status: Phase 0 complete** — the v0 script has been restructured into the
> `audiobooker/` package, the voice-cloning path removed, and a test suite seeded.
> Pipeline *quality* is still v0-level; the rebuild plan lives in [`docs/`](docs/):
>
...
> 4. [Implementation roadmap](docs/04-roadmap.md) — 6 phases with acceptance criteria; **next: Phase 1**
...
- `audiobooker/` — the package: `cli.py`, `binaries.py` (Tesseract/ffmpeg discovery),
  `extract/` (rasterize + OCR), `segment/` (cleanup + chapter split),
